        ...
        FastAutoGenerator.create("jdbc:mysql://localhost:3306/crm?serverTimezone=UTC", "root", "root")
                .globalConfig(builder -> {
                    builder.author("briup") // 设置作者  代码注释
                            .outputDir("D:\\crm\\crm-system\\src\\main\\java"); // 指定java代码输出目录
                })
                .packageConfig(builder -> {
                    builder.parent("com.briup") // 设置父包名
                            .moduleName("crmsystem") // 设置父包模块名
                            .pathInfo(Collections.singletonMap(OutputFile.xml, "D:\\crm\\crm-system\\src\\main\\resources\\mapper")); // 设置mapperXml生成路径
                })
                .strategyConfig(builder -> {
                    builder.addInclude("sys_user_auths") // 设置需要生成的表名
